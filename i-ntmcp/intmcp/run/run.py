...
def get_run_args_parser(parser: Optional[ArgumentParser] = None
    ...
    # )
    parser.add_argument(
        "-stl", "--step_time_limit", type=float, default=None,
        help=(
...
def parse_run_kwargs(args: Namespace) -> dict:
    ...
    run_kwargs = {
        ...
        "gamma": args.gamma,
        "pause": args.pause,
        # "u_const": args.u_const, # added by Daishi
        "step_time_limit": args.step_time_limit, # added by Daishi
    }
...
def main_run_with_args(model_class: Type[M.POSGModel],
    ...
    # cProfile.runctx('run_sims(model, policies, trackers, renderers, **run_kwargs)', globals(), locals(), '../cProfile_times3x3_7')

    # profiled = cProfile.Profile()
    # profiled.enable()
    # run_sims(model, policies, trackers, renderers, **run_kwargs)
    # profiled.disable()

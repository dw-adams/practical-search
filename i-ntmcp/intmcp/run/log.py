...
def make_dir(exp_name: str) -> str:
    ...
    # result_dir = os.path.join(BASE_RESULTS_DIR, f"{exp_name}")
    # print(''.join(str(datetime.now()).split(' ')))
    # print(''.join(str(datetime.now()).split('.')[0].split(':')))
    result_dir = os.path.join(BASE_RESULTS_DIR, f"{exp_name}_{''.join(str(datetime.now()).split('.')[0].split(':'))}")

...
import time
import math
import numpy as np # added by daishi
import random
...
class NestedSearchTree(policy.BasePolicy):
    ...
    def __init__(self,
                 ...
                 step_time_limit: float = None, # Added by daishi, to replace the line above 
                 **kwargs):
    ...
    def _reset_step_statistics(self):
        self._statistics = {
            "explored_nodes" : 0.0, # added by Daishi
            "simulations": 0.0, # added by Daishi
            "search_time": 0.0,
            "update_time": 0.0,
    ...
    def get_action(self) -> M.Action:
        ...
        start_time = time.time()
        simulation_calls = 0 # Added by Daishi
        for level in range(self.nesting_level+1):
            ...
            if time_limit is None: 
                ...
                simulation_calls += num_level_sims
            else:
                self._log_info2(f"Searching level={level} for time_limit={level_time_limit}s")
                time_limit_start = time.time()
                simulation_count = 0
                while (time.time()- time_limit_start) < level_time_limit:
                    self._nested_sim(self.history, level, True)
                    simulation_count += 1  
                simulation_calls += simulation_count
                

        search_time = time.time() - start_time
        search_time_per_sim = search_time / self.num_sims
        self._statistics["simulations"] += simulation_calls # added by Daishi
        self._log_info1(f"Total simulation calls={simulation_calls}")
        self._statistics["explored_nodes"] = 0 # added by Daishi, not expected to be used by this class
    ...
    def sample_action(self,
        ...
        a_probs = [
            np.exp(a_node.n**temperature / obs_n_sqrt) # changed by daishi
    ...
    def get_pi_by_history(self,
        ...
        temp = 1.0
        pi = {
            a_node.h: np.exp(a_node.n**temp / obs_n_sqrt) # math.exp(a_node.n**temp / obs_n_sqrt) # added by daishi
            for a_node in obs_node.children
        }
    ...
    def initialize(cls,
                   ...
                   step_limit_time: Union[float, List[float]] = None, # added by Daishi
        ...
        if nesting_level > 0:
            for i in range(model.num_agents):
                ...
                nested_tree = cls.initialize(
                    ...
                    uct_c=uct_c,
                    step_limit_time = step_limit_time, # added by Daishi
                    **kwargs
                )
        ...
        if isinstance(step_limit_time, list): # added by Daishi

...
from intmcp.model.history import AgentHistory, JointHistory # added
from intmcp.run.log import simulation_end
...
class NestedSearchTree(policy.BasePolicy):
    ...
    def __init__(self,
                 ...
                 epsilon: float = 0.01,
                 u_const: float = 0.0,
                 step_limit_time: float = None,
                 step_time_limit: float = None, # to replace the line above
                 **kwargs):
        ...
        self.step_limit_time = step_limit_time
        self.step_time_limit = step_time_limit
        if step_time_limit is not None:
            self.step_limit_time = step_time_limit
    ...
    def _prune_traverse(self,
        ...
        if len(node_history.history) == horizon:
            if node_history not in histories:
                self._log_debug1(f"horizon={horizon} pruning history={node_history}")
                del node
            return
        ...
        for action_node in node.children:
            for obs_node in action_node.children:
                ...
                if horizon-2 > 0 and len(history_tp1.history) == horizon-2:
                    self._log_debug2(
                        f"horizon={horizon} clearing obs node for "
    ...
    def get_action(self) -> M.Action:
        ...
        time_limit = self.step_limit_time
        # print(f"time {time_limit}")
        if time_limit is None:
            self._log_info1(f"Searching for num_sims={self.num_sims}")
        else:
            self._log_info1(f"Searching for sims_time_limit={time_limit}s")
            # allow for equal time for each level
            level_time_limit = time_limit / (self.nesting_level+1)
        ...
        for level in range(self.nesting_level+1):
            if time_limit is None: 
                ...
                self._log_info2(f"Searching level={level} for num_level_sims={num_level_sims}")
                exploration_count = [0,0,0]

                # reset the node list, for each set of simulations
                self.reset_node_list()
                for i in range(num_level_sims): # here, we are simulating each level n = 1000 times
                    self._nested_sim_new(exploration_count, i ,self.history,level,True)
                    # self._nested_sim_new(exploration_count, i ,self.history,level,True)
                    # self._nested_sim(i, self.history, level, True)
                    # self._nested_sim_v2(i, self.history, level, True)

                    # account for the extra simulations that occur when exploring higher nesting levels
                    # Note, this is not necessary for the original code
                    # if exploration_count[2] >= num_level_sims:
                    #     break
                simulation_calls += exploration_count[2]
                total_explored_nodes += exploration_count[0]
            else: # limit the search time at each level
                self._log_info2(f"Searching level={level} for time_limit={level_time_limit}s")
                ...
                # num_level_sims = self._get_level_sims(level)
                simulation_count = 0
                while (time.time()- time_limit_start) < level_time_limit: # here, we are simulating each level n = 1000 times
                    self._nested_sim_new(exploration_count, simulation_count ,self.history,level,True)                 
                    simulation_count += 1  
                simulation_calls += exploration_count[2]
                total_explored_nodes += exploration_count[0] 
            
        self._log_debug2(f"timed:{self.step_time_limit}")
        search_time = time.time() - start_time
        search_time_per_sim = search_time / self.num_sims
        self._statistics['explored_nodes'] += total_explored_nodes
        self._statistics['simulations'] += simulation_calls
        self._log_info1(f"Total simulation calls={simulation_calls}")
        self._statistics["search_time"] += search_time
        self._log_info1(f"search_time={search_time:.2f} s, search_time_per_sim={search_time_per_sim:.5f}")
    ...
    # This is the current algorithm being used
    def ucb_expansion_action(self, 
                        exp_count,
                        iteration,
                        history : M.AgentHistory,
                        root_node: Node):
        # get all accessible nested tree nodes from this point
        ucb_constant = self.u_const #0.5 #self.ucb_const
        traversal_depth = len(history.history)
        for i, nested_tree in self.nested_trees.items():
            # if iteration == 0:
            #     # set the existing values
            
            # construct the node list on the first run

            if iteration == 0:
                possible_beliefs = nested_tree.get_all_traverse_action(traversal_depth)
                self.node_list = possible_beliefs
                # belief_values = [
                #     tree.v +
                #     ucb_constant * np.sqrt( np.log(root_node.n, where = root_node.n > 0.0)/ (tree.n+0.1))
                #     for tree in possible_beliefs
                # ]
                # save as persistant variable
                # self.belief_list = belief_values
            else:
                possible_beliefs = self.node_list
                # belief_values = self.belief_list
            # possible_beliefs = nested_tree.get_all_traverse_action(traversal_depth)
            # once the action trees are obtained, select the best tree
            belief_values = [
                tree.v +
                ucb_constant * np.sqrt( np.log(root_node.n, where = root_node.n > 0.0)/ (tree.n+0.1))
                for tree in possible_beliefs
            ]
            # Used to evaluate exploration amount
            # todo, remove this time from the search time, as this is evaluation
            greedy_values = [
                tree.v for tree in possible_beliefs
            ]
            tree_index = np.argmax(belief_values)
            # count exploration vs exploitation: exploration when non-max value is selected
            if tree_index != np.argmax(greedy_values):
                exp_count[0] = exp_count[0] + 1
                # print("explored", exp_count)
            exp_count[1] = exp_count[1] + 1
            exp_count[2] = exp_count[2] + 1

            # obtain the best node to expand
            tree_node = possible_beliefs[tree_index].parent
            h_state = tree_node.belief.sample()
            assert isinstance(h_state, HistoryState)
            
            for j in range(nested_tree.num_agents):
                h_j = h_state.history.get_agent_history(j)
                if j == nested_tree.ego_agent:
                    nested_tree._rollout_policy.reset_history(h_j)
                elif nested_tree.nesting_level > 0:
                    nested_tree.nested_trees[j]._rollout_policy.reset_history(h_j)
            nested_tree.simulate(h_state, tree_node, 0)
            tree_node.n += 1
            # print(tree_node.n)
            # recalculate the values for the given node:
            # valued_node = possible_beliefs[tree_index]
            # new_value = valued_node.v + ucb_constant * np.sqrt( np.log(root_node.n, where = root_node.n > 0.0)/ (valued_node.n+0.1))
            # belief_values[tree_index] = new_value
            
            # update the node with the new value
            # self.belief_list[tree_index] = new_value
            # self.belief_list = belief_values
        return

    # This is the current algorithm being used
    def ucb_simulation_approximate(self, 
                        exp_count,
                        iteration,
                        history : M.AgentHistory,
                        root_node: Node):
    ...
    def ablation_simulation(self, 
        ...
        ucb_constant = self.u_const #0.5 #self.ucb_const
        traversal_depth = len(history.history)
        # select some node in the lower level tree uniformly.
        h_state = root_node.belief.sample()
        for i, nested_tree in self.nested_trees.items():
            lower_history = h_state.history.get_agent_history(i)
            tree_node = self.traverse(lower_history)

            if len(tree_node.children) == 0:
                self.expand(tree_node, history) 

            # simulate the selected node
            h_state = tree_node.belief.sample() # these beliefs may not be developed, which is causing the method to break. Or it attempts to access a level -1 node
            for j in range(nested_tree.num_agents):
                h_j = h_state.history.get_agent_history(j)
                if j == nested_tree.ego_agent:
                    nested_tree._rollout_policy.reset_history(h_j)
    ...
    def _nested_sim_new(self, 
        ...
        self._log_debug2(f"nested_sim: search_level={search_level}")
        root = self.traverse(history)
       
        if len(root.children) == 0:
            self.expand(root, history)
        self._handle_depleted_node(history, root, top_level)
        h_state = root.belief.sample() # original: # Right here.... which samples from particles
        assert isinstance(h_state, HistoryState)
        if self.nesting_level > search_level:         
            # insert new function here:
            # self.ucb_expansion_action(exp_count, iteration_value, history, root) # simulation with true ucb
            self.ucb_simulation_approximate(exp_count, iteration_value, history, root) # simulation with approx ucb
            # self.ablation_simulation(exp_count, iteration_value, history, root) # simulation used as ablation study
            for i, nested_tree in self.nested_trees.items():
                nested_tree._nested_sim_new( exp_count, iteration_value,
    ...
    # added: to reset the node list at the beginning of each simulation round
    def reset_node_list(self) -> None:
        self.node_list = []

    def set_node_list(self, node_list):
        self.node_list = node_list

    # get some of the statistics for the number of explorations, expect smaller std for more explorations
    def get_ns(self, node_list):
        
        # get the n values
        ns = np.array([node.n for node in node_list])
        ...
        # attempt to grab all nodes at some time t - depth of the tree
        # This one contains the history of each node, which uses more memory.
    def get_all_traverse(self, depth: int):
        # h_nodes = [(self.root,[])]
        h_nodes = {self.root : []}
        for _ in range(depth):
            # also need to record the histories
            # children = []
            children = {}
            # This should provide all childen due to an action
            for h_node in h_nodes.items():
                # print(h_node)
                for act_child in h_node[0].children:
                    act_hist = act_child.h
                    # all observation children of action
                    for obs_child in act_child.children:
                        obs_hist = obs_child.h
                        new_history = h_node[1] + [(act_hist,obs_hist)]
                        children[obs_child] = new_history
            h_nodes = children
        # retuns a dictionary of all nodes at some time t, and the respective history to reach the state
        return h_nodes

        # attempt to grab all nodes at some time t - depth of the tree
    def get_all_traverse_no_h(self, depth: int):
        h_nodes = [self.root]
        for _ in range(depth):
            children = []
            # This should provide all childen due to an action
            for h_node in h_nodes:
                children += h_node.children
            h_nodes = children
            children = []
            # And gain all children, for the observation nodes
            for h_node in h_nodes:
                # for child in h_node.children: # only add children with beliefs
                #     if child.belief.size() != 0:
                #         children.append(child)
                children += h_node.children
            h_nodes = children
        # And ensure that the children have action children
        b_size = 0
        for node in h_nodes:
            if node.belief.size() == 0:
                b_size += 1
            if len(node.children) == 0:
                history = self.traverse_up(node)
                self.expand(node, history)
        return h_nodes

    def get_all_traverse_action(self, depth: int):
        # h_nodes = [(self.root,[])]
        h_nodes = [self.root]
        for _ in range(depth):
            children = []
        ...
        for node in h_nodes:
            ...
            if len(node.children) == 0:
                history = self.traverse_up(node)
                self.expand(node, history)
            # return these action nodes
            actions += node.children
        return actions

    # traverses up the tree to construct a history
    def traverse_up(self, node):
        h_node = node
        history = []
        while not h_node.is_root():# != self.root:
            obs_h = h_node.h
            act_h = h_node.parent.h
            
            history.append(tuple([act_h, obs_h]))
            
            h_node = h_node.parent.parent         
        # and reverse history, such that it's in order
        history = M.AgentHistory(history[::-1])
        return history

    def _nested_sim(self, sim_number,
                    history: M.AgentHistory,
                    search_level: int,
                    top_level: bool = False):
        """Run nested simulation on this policy tree starting from history """
        ...
        h_state = root.belief.sample() # original: # Right here.... which samples from particles
        assert isinstance(h_state, HistoryState)
        if self.nesting_level > search_level: # need to optimise this part?          
            for i, nested_tree in self.nested_trees.items():
                nested_tree._nested_sim( sim_number,
                    h_state.history.get_agent_history(i), search_level, False
                )
        else:
            for i in range(self.num_agents):
    ...
    def get_action_by_history(self, history: M.AgentHistory) -> M.Action:
        ...
        obs_node = self.traverse_no_add(history)
        if obs_node is None:
            return random.choice(self.action_space)
        return self.get_action_from_node(obs_node, history, True)
    
    def get_action_by_history_no_add(self, history: M.AgentHistory) -> M.Action:
        """Get action from policy for given history """
        obs_node = self.traverse_no_add(history)
        if obs_node is None:
            return random.choice(self.action_space)
    ...
    def sample_action(self,
        ...
        a_probs = [
            np.exp(a_node.n**temperature / obs_n_sqrt)
    ...
    def traverse_no_add(self, history: M.AgentHistory) -> Node:
        """Traverse policy tree and return node corresponding to history """
        h_node = self.root
        for (a, o) in history:
            if h_node.has_child(a):
                if h_node.get_child(a).has_child(o):
                    h_node = h_node.get_child(a).get_child(o)
                else: # raise an error
                    return None
            else: # raise an error
    ...
    def _handle_depleted_node(self,
        ...
        if b_size == 0 or (b_size < self._extra_particles and top_level):
            self._log_debug(
                f"depleted h_node={h_node} b_size={b_size} top_level={top_level}"
    ...
    def _reinvigorate_belief(self,
        ...
        if h_node_size is not None and h_node_size < target_node_size:
            self._log_debug1((
                ...
                f"parent_obs_node={parent_obs_node} to h_node={h_node} for history={history}. "
    ...
    def _rejection_sample(self,
        ...
        """
        self._log_debug2(f"Sampling num_samples={num_samples} for history={history}")
    ...
    def get_pi_by_history(self,
        ...
        temp = 1.0
        pi = {
            a_node.h: np.exp(a_node.n**temp / obs_n_sqrt)
            for a_node in obs_node.children
        }
    ...
    def get_args_parser(cls,
        ...
        # added by Daishi
        parser.add_argument(
            "--u_const", type=float, default=0.,
            help="UCB search constant (default=0.0)"
        )
        parser.add_argument(
            "-slt", "--step_limit_time", nargs="*", type=float, default=[None],
            help=(
                "Time limit for each step, replaces step_limit."
                "(default = None)"
            )
    )
        return parser
    ...
    def initialize(cls,
                   ...
                   step_limit_time: Union[float, List[float]] = None,
        ...
        if nesting_level > 0:
            for i in range(model.num_agents):
                ...
                nested_tree = cls.initialize(
                    ...
                    uct_c=uct_c,
                    step_limit_time = step_limit_time,
        ...
        if isinstance(step_limit_time, list):
            if len(step_limit_time) == 1:
                step_limit_time = step_limit_time[0]
            else:
                step_limit_time = step_limit_time[ego_agent]
        print("timelimit:", kwargs)
        # if step_time_limit is not None:
        #     step_limit_time = step_time_limit
